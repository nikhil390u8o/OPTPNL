...
async def _sign_in(phone, code, phone_code_hash):
    ...
    # Check both admin key (phone) and user key (usr_phone)
    data   = tg_clients.get(phone) or tg_clients.get(f"usr_{phone}", {})
    client = data.get("client") if data else None
    # Client nahi mila — recreate karo with stored hash
    if not client:
        client = Client(f"sign_{phone.replace('+','')}", api_id=API_ID, api_hash=API_HASH, in_memory=True)
        await client.connect()
    try:
        ...
        # Keep client for 2FA — store with phone key
        tg_clients[phone] = {"client": client, "hash": phone_code_hash}
...
async def _check_2fa(phone, password):
    ...
    client = data.get("client") if data else None
    if not client:
        raise Exception("Session expired. Dobara Send OTP karo.")
...
async def _listen_for_otp(order_id, string_session, phone):
    ...
    try:
        ...
        import re as _re
        print(f"🎧 Starting OTP listener for order {order_id}, phone {phone}")
        client = Client(
            f"listen_{order_id}",
            api_id=API_ID,
            api_hash=API_HASH,
        ...
        await client.start()
        print(f"✅ Pyrogram connected for order {order_id}")
        ...
        async def on_msg(c, m):
            ...
            print(f"📨 Message received for order {order_id}: {text[:50]}")
            match = _re.search(r"\b(\d{5})\b", text)
            if match:
                otp = match.group(1)
                otp_found = True
                print(f"🔑 OTP found for order {order_id}: {otp}")
                # Save to DB
                db_query("UPDATE orders SET otp=%s, status='otp_received' WHERE id=%s", (otp, order_id))
                # Emit via socketio
                socketio.emit("otp_received", {"otp": otp}, room=str(order_id), namespace="/")
                try:
                    await client.stop()
                except:
                    pass
        ...
        if not otp_found:
            print(f"⏰ OTP timeout for order {order_id}")
            try:
                await client.stop()
            except:
                pass
    except Exception as e:
        print(f"❌ OTP listener error for order {order_id}: {e}")
        import traceback
        traceback.print_exc()
...
def get_otp(order_id):
    """Polling endpoint - check if OTP received"""
